...
import com.example.thuva.api.exceptions.CustomerNotFoundException;
import com.example.thuva.api.exceptions.GenericAlreadyExistsException;
import com.example.thuva.api.model.Item;
import com.example.thuva.api.repository.CartRepository;
import com.example.thuva.api.repository.UserRepository;
    ...
    public List<Item> addCartItemByCustomerId(String customerId, @Valid Item item) {
        ...
        long count = entity.getItems().stream()
                .filter(i -> i.getProduct().getId()
                        .equals(UUID.fromString(item.getId()))).count();
        if (count > 0) {
...
//            items = List.of();
    ...
    public Item getCartItemByItemId(String customerId, String itemId) {
        ...
        if (Objects.isNull(itemEntity.get())) {
            getUnsafe().throwException(new CustomerNotFoundException.ItemNotFoundException(String.format(" - %s", itemId)));
        }
